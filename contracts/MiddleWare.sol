...
contract MiddleWare {
  ...
  function getNftOwnerAndTokenURI(
    address nftAddr,
    uint256 nftId
  ) public view returns (address owner, string memory uri) {
    IERC721Metadata nft = IERC721Metadata(nftAddr);
    try nft.ownerOf(nftId) returns (address ownerAddr) {
      owner = ownerAddr;
    } catch {}
  ...
  function getDoNftMarketInfo(
    ...
    if (doNft.exists(nftId)) {
      (uint256 oid, , uint64[] memory starts, uint64[] memory ends, ) = doNft
        .getDoNftInfo(nftId);
      ...
      if (doNftInfo.orderIsValid) {
        IMarket.Lending memory order = market.getLendOrder(nftAddr, nftId);
        ...
        if (
          ...
          doNftInfo.orderPrivateRenter = market.getRenterOfPrivateLendOrder(
            nftAddr,
            nftId
          );
        }
  ...
  function batchIsApprovedForAll(
    address owner,
    address[] calldata operators,
    address[] calldata erc721Array
  ) external view returns (bool[] memory results) {
    results = new bool[](erc721Array.length);
    for (uint i = 0; i < erc721Array.length; i++) {
      results[i] = IERC721(erc721Array[i]).isApprovedForAll(
        owner,
        operators[i]
      );
    }
  ...
  function batchGetDoNftIdByONftId(
    address[] calldata doNftAddressArray,
    uint256[] calldata oNftIdArray
  ) external view returns (uint256[] memory doNftIdArray) {
    require(
      doNftAddressArray.length == oNftIdArray.length,
      "invalid input data"
    );
