...
contract TimeLockMultiSigWallet {
  ...
  function batchConfirmTransaction(uint256[] memory transactionIdArray) public {
  ...
  /// @param transactionIdArray Transaction ID array.
  function batchExecuteTransaction(uint256[] memory transactionIdArray) public {
    for (uint256 i = 0; i < transactionIdArray.length; i++) {
  ...
  {
    require(
      getBlockTimestamp() >= transactions[transactionId].submitTime + delay,
