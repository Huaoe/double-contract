...
contract ComplexDoNFT is WrapDoNFT, Royalty, IComplexDoNFT {
  ...
  function initialize(
    ...
    super._BaseDoNFT_init(name_, symbol_, nftAddress_, market_, owner_, admin_);
    royaltyAdmin = royaltyAdmin_;
  }

  function mintVNft(
    uint256 oid
  ) public virtual override nonReentrant returns (uint256 tid) {
    ...
    if (IERC165(oNftAddress).supportsInterface(type(IWrapNFT).interfaceId)) {
      ...
      lastOwner = IERC721(gameNFTAddress).ownerOf(oid);
      if (lastOwner != oNftAddress) {
        require(
          onlyApprovedOrOwner(msg.sender, gameNFTAddress, oid),
          "only approved or owner"
        );
        IERC721(gameNFTAddress).safeTransferFrom(lastOwner, address(this), oid);
        IERC721(gameNFTAddress).approve(oNftAddress, oid);
        ...
        lastOwner = IERC721(oNftAddress).ownerOf(oid);
        IERC721(oNftAddress).safeTransferFrom(lastOwner, address(this), oid);
      }
    } else {
      require(
        onlyApprovedOrOwner(msg.sender, oNftAddress, oid),
  ...
  function redeem(
    ...
    if (IERC165(oNftAddress).supportsInterface(type(IWrapNFT).interfaceId)) {
      ...
      address gameNFTAddress = IWrapNFT(oNftAddress).originalAddress();
      IERC721(gameNFTAddress).safeTransferFrom(
        address(this),
        ownerOf(tokenId),
        info.oid
      );
  ...
  function tokenURI(
    ...
    string memory originalSymbol = IERC721Metadata(oNftAddress).symbol();

    if (IERC165(oNftAddress).supportsInterface(type(IWrapNFT).interfaceId)) {
      address gameNFTAddress = IWrapNFT(oNftAddress).originalAddress();
      originalSymbol = IERC721Metadata(gameNFTAddress).symbol();
    }
    ...
    return
      DoubleSVG.genTokenURI(tokenId, name_, type_value, start_time, end_time);
  }
}
