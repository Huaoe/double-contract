...
contract Market is OwnableContract, ReentrancyGuardUpgradeable, IMarket {
  ...
  function getRenterOfPrivateLendOrder(
    address nftAddress,
    uint256 nftId
  ...
  function getPaymentNormal(
    address nftAddress,
    uint256 nftId
  ) external view returns (PaymentNormal memory) {
    ...
    )
  {
    ...
    if (pNormal.token == address(0)) {
      ...
      uint256 balance_before = IERC20(pNormal.token).balanceOf(address(this));
      ...
      uint256 balance_after = IERC20(pNormal.token).balanceOf(address(this));
      require(
  ...
  function setMarketBeneficiary(address payable beneficiary_) public onlyOwner {
